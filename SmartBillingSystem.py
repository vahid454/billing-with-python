...
#-------------Customer Field Listener
customerName=""


def customerFieldListener(a,b,c):
    global customerVar
    global quantityVar
    global costVar
    global itemRate
    customer = customerVar.get()
    if customer != "":
        try:
            customer=customer
            
            customerVar.set("%s"%customer)
            
        except ValueError:
            customer=customer[:-1]
            customerVar.set(customer)
    else:
        customer=""
        customerVar.set(" ")
...
imemVar=StringVar()
imemVar.trace('w',imemFieldListener)


customerVar=StringVar()
customerVar.trace('w',customerFieldListener)
...
#============= add item Variables==========
storeOptions=['Frozen','Fresh','Refurbished','Second-Hand','Old']
...
def generate_bill():
    ...
    cost = costVar.get()
    conn = pymysql.connect(host="localhost", user="root", passwd="vahid999", db="billservice")
    cursor = conn.cursor()
...
def print_bill():
    ...
    print(imem);
    print(imemItem)    
    customerName=customerVar.get();
    x=datetime.date.today()

    billString = ""

    billString+="===================SMART PLAZA========================\n\n"

    billString+="=====================Receipt==========================\n\n"
    billString+="Name: {0}             Date : {1}\n".format(customerName,x)
    ...
    billString+="                       Pro. Rasheed Mansuri 7000981362\n"
    ...
    if billFile is None:
    	...
    	billFile.close()
    	print(billFile.name)
    	os.system("Text2Pdf.py {0}".format(billFile.name))
    	
        
    print(billString)
    
    # here the work of pdf
    # with open(billFile, 'wb') as out:
...
def addItem():
    ...
    conn.commit()
    conn.close()
    imemItem.append("Mi A1");
    if Type=="Mobile":
    	imemItem.append(name);
    addItemNameVar.set("")
    addItemRateVar.set("")
    addItemTypeVar.set("")
def updateItem():
...
def loginWindow():
    titleLabel = Label(window,text="   Smart Plaza Billing System",font="Arial 40",fg="green")
    titleLabel.grid(row=0,column=3,columnspan=4, padx=(40,0),pady=(10,0))

    loginLabel = Label(window,text="Admin Login",font="Arial 30")
    loginLabel.grid(row=1, column=3,padx=(40,0),columnspan=4, pady=10)

    usernameLabel = Label(window, text="Username")
    usernameLabel.grid(row=2, column=2,columnspan=4,padx=10,pady=8)

    passwordLabel = Label(window, text="Password")
    passwordLabel.grid(row=3, column=2,columnspan=4,padx=10,pady=8)

    usernameEntry= Entry(window, textvariable=usernameVar)
    usernameEntry.grid(row=2, column=5,padx=10,pady=8)

    passwordEntry = Entry (window, textvariable=passwordVar,show="*")
    passwordEntry.grid(row=3,column=5,padx=10,pady=8)

    loginButton=Button(window, text="Login",width=20, height=2, command=lambda:adminLogin())
    loginButton.grid(row=4, column=4, columnspan=2)

def mainwindow():
    
    titleLabel = Label(window,text=" Smart Plaza Billing System",font="Arial 30",fg="green")
    ...
    customerLabel.grid(row=2, column=0, padx=(5,0), pady=(5,0))
    customerEntry=Entry(window, textvariable=customerVar)
    customerEntry.grid(row=2, column=1, padx=(5,0), pady=(10,0))

    itemLabel = Label(window, text="Select Item")
    itemLabel.grid(row=3, column=0, padx=(5,0),pady=(10,0))

    itemDropDown=OptionMenu(window,itemVariable,*options,command=optionMenuListener)
    itemDropDown.grid(row=3, column=1,padx=(10,0), pady=(10,0))

    rateLabel= Label(window, text="Rate")
    rateLabel.grid(row=3, column=2, padx=(10,0), pady=(10,0))

    rateValue = Label(window, textvariable=rateVar)
    rateValue.grid(row=3, column=3, padx=(10,0), pady=(10,0))

    quantityLabel = Label(window, text="Quantity")
    quantityLabel.grid(row=4, column=0,padx=(5,0),pady=(10,0))
    quantityEntry=Entry(window, textvariable=quantityVar)
    quantityEntry.grid(row=4, column=1,padx=(5,0),pady=(10,0))


    costLabel= Label(window, text="Cost")
    costLabel.grid(row=4, column=2, padx=(10,0), pady=(10,0))

    costEntry=Entry(window, textvariable=costVar)
    costEntry.grid(row=4, column=3, padx=(10,0), pady=(10,0))

    imemLabel = Label(window, text="IMEM NO. : ")
    imemLabel.grid(row=5, column=0,padx=(5,0),pady=(10,0))
    imemEntry=Entry(window, textvariable=imemVar)
    imemEntry.grid(row=5, column=1,padx=(5,0),pady=(10,0))
 

    buttonBill = Button(window, text="Add to List", width =15,command=lambda:generate_bill())
    buttonBill.grid(row=5, column=4,padx=(5,0),pady=(10,0))

    billLabel=Label(window, text="Bills", font="Arial 25")
    billLabel.grid(row=6, column=2)

    billsTV.grid(row=7, column=0, columnspan=5)
    ...
    generateBill.grid(row=8,column=4)
...
def itemAddWindow():
    ...
    titleLabel = Label(window,text=" Smart Plaza Billing System", width=40,font="Arial 30",fg="green")
